...
# Check if local glomap:local image exists (in case you ran build.sh), otherwise use official colmap image
if docker image inspect glomap:local >/dev/null 2>&1; then
    echo "Using local glomap Docker image..."
    GUI_IMAGE="glomap:local"
else
    echo "Local GLOMAP image not found, pulling official COLMAP image..."
    docker pull colmap/colmap:latest
    GUI_IMAGE="colmap/colmap:latest"
fi
...
if docker run --rm --runtime=nvidia "${GUI_IMAGE}" nvidia-smi >/dev/null 2>&1; then
...
# Pass all arguments after the directory path ("${@:2}") to the glomap gui command.
echo "Launching GUI..."
# "${@:2}" accepts extra arguments for the GLOMAP GUI.
docker run "${DOCKER_ARGS[@]}" "${GUI_IMAGE}" colmap gui "${@:2}"
